object LearningScala3 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(225); 
  ...
  transformInt(2, x => {val y = x * 2; y * y});System.out.println("""res2: Int = """ + $show(res$2));$skip(508); 
  ...
  def stringToUpper(s:String):String = {
  	s.toUpperCase()
  };System.out.println("""stringToUpper: (s: String)String""");$skip(34); 
  val dkStr = stringToUpper("dk");System.out.println("""dkStr  : String = """ + $show(dkStr ));$skip(76); 
  
  def transformString(s:String, f:String=>String):String = {
  	f(s)
  };System.out.println("""transformString: (s: String, f: String => String)String""");$skip(66); 
  
  val upS = transformString("dharmik", x => {x.toUpperCase()});System.out.println("""upS  : String = """ + $show(upS ))}
  //--------------------------------------------------------------------------------------
}
